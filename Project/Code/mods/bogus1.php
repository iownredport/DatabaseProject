...
$myPhpVar= '<script>stwing</script>';
  ...
  $sql2 = ("select exists (select * from $d where ($c)=($variable)) as chopstick");

$result69 = $conn->query($sql2);
	while ($row69 = $result69->fetch_assoc()) {
			if($row69['chopstick'] == 0){
		die ("What you are trying to delete already does not exist."); 
	}
 ...
 $sql1 = ("delete from $d where ($c)=($variable)");
...
");
	...
	while ($row1 = $result1->fetch_assoc()) {
			if($row1['ver'] == 0){
		$delete1 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete1);
...
$result2 = $conn->query($pattern2);
	while ($row2 = $result2->fetch_assoc()) {
			if($row2['per'] == 0){
		$delete2 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete2);
...
$result3 = $conn->query($pattern3);
	while ($row3 = $result3->fetch_assoc()) {
			if($row3['jep'] == 0){
		$delete3 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete3);
...
(select * from sells where item not in (select food from items) and item not in (select beer from items)) as tickle");
	...
	while ($row5 = $result5->fetch_assoc()) {
		if($row5['tickle'] == 0){
		$delete4 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete4);
...
where sells.price != bills3.foodprice and sells.barname = bills3.barname and sells.item = bills3.food) as lemon");
	...
	while ($row6 = $result6->fetch_assoc()) {
		if($row6['lemon'] == 0){
		$delete6 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete6);
		die ("Error: Please make sure the correct food/beer price exists."); 
	}
...
having tax1 != tax+0.01 xor tax != tax-0.01 xor tax1 != tax) as pipe");
	...
	while ($row7 = $result7->fetch_assoc()) {
		if($row7['pipe'] == 0){
		$delete7 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete7);
		die ("Error: Please make sure the correct 7% sales tax exists."); 
	}
...
having total != totalcost+0.01 xor total != totalcost-0.01 xor total != totalcost) as jaden");
	...
	while ($row8 = $result8->fetch_assoc()) {
		if($row8['jaden'] == 0){
		$delete8 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete8);
		die ("Error: Please make sure the correct total cost exists"); 
	}
...
$result23 = $conn->query($gdax);
	while ($row23 = $result23->fetch_assoc()) {
			if($row23['thot'] == 0){
		$delete23 = ("insert into $d ($c) values ($variable)");
		$conn->query($delete23);
		die ("You shall not delete a query in the sells table that is in the bills3 table."); 
	}
